...
function loadMusic(indexNumb) {
    const musicData = allMusic[indexNumb - 1];
    musicName.innerText = musicData.name;
    musicArtist.innerText = musicData.artist;
    musicImage.src = `img/${musicData.img}.jpg`;
    
    // Verifica se a música possui um link externo
    if (musicData.link) {
        mainAudio.src = musicData.link;
    } else {
        mainAudio.src = `music/${musicData.src}.mp3`;
    }
}
